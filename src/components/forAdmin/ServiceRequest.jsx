  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
    <div className="bg-secondary rounded-2xl w-full max-w-sm shadow-xl">
      ...
      <div className="px-6 py-5 space-y-2">
        ...
        {action === "approve" && <p className="text-primary/40 text-xs">A project will be automatically created upon approval.</p>}
      </div>
      <div className="px-6 py-4 border-t border-primary/8 flex justify-end gap-2">
        ...
        <button onClick={() => onConfirm(request._id, action)} disabled={isPending} className={`px-4 py-2 text-sm font-semibold rounded-lg text-white transition disabled:opacity-50 ${action === "approve" ? "bg-green-500 hover:bg-green-600" : "bg-red-500 hover:bg-red-600"}`}>
...
const ServiceRequest = () => {
  ...
  return (
    <div className="space-y-4">
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((req) => (
                <tr key={req._id} className="border-b border-primary/5 last:border-0 hover:bg-primary/2 transition">
                  ...
                  <td className="px-5 py-3">
                    {req.status === "pending" && !req.client?.isDeleted ? (
                      <div className="flex items-center gap-2">
                        <button onClick={() => setConfirm({ action: "approve", request: req })} className="text-xs px-3 py-1 rounded-lg border border-green-200 text-green-600 hover:bg-green-500 hover:text-white font-semibold transition">
                          Approve
                        </button>
                        <button onClick={() => setConfirm({ action: "reject", request: req })} className="text-xs px-3 py-1 rounded-lg border border-red-200 text-red-500 hover:bg-red-500 hover:text-white font-semibold transition">
                          Reject
                        </button>
                      </div>
                    ) : (
      ...
      {confirm && <ConfirmModal action={confirm.action} request={confirm.request} isPending={isPending} onConfirm={(id, action) => handleAction({ id, action })} onClose={() => setConfirm(null)} />}
    </div>
  );
};
