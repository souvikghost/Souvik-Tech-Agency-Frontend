  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
    <div className="bg-secondary rounded-2xl w-full max-w-sm shadow-xl">
      ...
      <div className="px-6 py-5 flex flex-col items-center gap-4">
        <div className="w-16 h-16 rounded-full bg-primary text-secondary text-xl font-bold flex items-center justify-center">
      ...
      <div className="px-6 py-4 border-t border-primary/8 flex justify-end gap-2">
        <button onClick={onClose} className="px-4 py-2 text-sm font-semibold text-primary/60 hover:text-primary transition">Close</button>
        {project.status === "in-progress" && (
          <button onClick={() => onRemove(project, employee)} disabled={isPending}
            className="px-4 py-2 text-sm font-semibold bg-red-500 hover:bg-red-600 text-white rounded-lg transition disabled:opacity-50">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="bg-secondary rounded-2xl w-full max-w-md shadow-xl">
        ...
        <div className="px-6 py-5 space-y-2 max-h-72 overflow-y-auto">
          {allEmployees.length === 0 ? (
            ...
            <label key={emp._id} className="flex items-center gap-3 p-3 rounded-xl border border-primary/8 hover:bg-primary/4 cursor-pointer transition">
              ...
              <div className="w-8 h-8 rounded-full bg-primary text-secondary text-xs font-bold flex items-center justify-center shrink-0">
        ...
        <div className="px-6 py-4 border-t border-primary/8 flex justify-end gap-2">
          ...
          <button onClick={() => onSave(project._id, selected)} disabled={isPending}
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
    <div className="bg-secondary rounded-2xl w-full max-w-sm shadow-xl">
      ...
      <div className="px-6 py-5 space-y-2">
        <p className="text-primary text-sm">Mark this project as <span className="font-bold text-emerald-600">successfully completed</span>?</p>
      ...
      <div className="px-6 py-4 border-t border-primary/8 flex justify-end gap-2">
        <button onClick={onClose} className="px-4 py-2 text-sm font-semibold text-primary/60 hover:text-primary transition">Cancel</button>
        <button onClick={() => onConfirm(project._id)} disabled={isPending}
          className="px-4 py-2 text-sm font-semibold bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg transition disabled:opacity-50">
...
const ConfirmStopModal = ({ project, onConfirm, onClose, isPending }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
    <div className="bg-secondary rounded-2xl w-full max-w-sm shadow-xl">
      ...
      <div className="px-6 py-5 space-y-2">
        <p className="text-primary text-sm">Are you sure you want to <span className="font-bold text-red-500">stop</span> this project?</p>
        <div className="bg-primary/4 rounded-xl p-3 space-y-1">
          <p className="text-primary text-xs font-semibold">{project.name}</p>
          <p className="text-primary/50 text-xs">Client: {project.client?.name}</p>
        </div>
  ...
  <div className="bg-white border border-primary/8 rounded-2xl p-5 flex flex-col gap-4">
    <div className="flex items-start justify-between gap-2">
      <div>
        ...
        <p className="text-primary/40 text-xs mt-0.5">{project.client?.name}</p>
      </div>
      ...
      {project.employees?.length > 0 ? (
        <div className="flex items-center -space-x-2">
          {project.employees.map((emp) => (
            <div key={emp._id} title={emp.name} onClick={() => onSelectEmployee(emp, project)}
              className="w-8 h-8 rounded-full bg-primary text-secondary text-xs font-bold flex items-center justify-center cursor-pointer ring-2 ring-white hover:ring-primary/40 hover:scale-110 transition-all">
              {getInitials(emp.name)}
            </div>
          ))}
        </div>
    ...
    <div className="flex gap-2">
      {project.status === "in-progress" && (
        <>
          <button onClick={() => onAssign(project)}
            className="flex-1 text-xs py-2 rounded-lg border border-primary/15 text-primary font-semibold hover:bg-primary hover:text-secondary transition">
            Assign
          </button>
          <button onClick={() => onComplete(project)}
            className="flex-1 text-xs py-2 rounded-lg border border-emerald-200 text-emerald-600 hover:bg-emerald-500 hover:text-white font-semibold transition">
            Complete
          </button>
          <button onClick={() => onStop(project)}
            className="flex-1 text-xs py-2 rounded-lg border border-red-200 text-red-500 hover:bg-red-500 hover:text-white font-semibold transition">
            Stop
          </button>
        </>
      )}
    ...
    <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((project) => (
            <ProjectCard
              ...
              onSelectEmployee={(emp, proj) => setSelectedEmployee({ employee: emp, project: proj })}
            />
          ))}
        </div>
      )}
      ...
      {assignTarget && (
        <AssignModal
          ...
          onSave={(id, employees) => saveAssign({ id, employees })}
        />
      )}
